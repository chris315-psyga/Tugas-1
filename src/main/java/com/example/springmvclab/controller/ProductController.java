...
public class ProductController {
    ...
    // Endpoint untuk menampilkan ringkasan kategori
    ...
    public String listCategories(Model model) {
        ...
        for (String category : categories) {
            int count = productService.findByCategory(category).size();
            categorySummary.put(category, count);
        }

        model.addAttribute("categorySummary", categorySummary);
        model.addAttribute("title", "Ringkasan Kategori");

        return "product/categories"; // -> templates/product/categories.html
    }
}